...
def csi_data_read_parse(port: str, csv_writer, log_file_fd,callback=None):
    ...
    if set.isOpen():
        ...
        return
    while True:
        raw_line = set.readline()
        if not raw_line:
            break
        strings = raw_line.decode('utf-8', errors='replace').rstrip('\r\n')
        ...
        if not header_written:
            csv_writer.writerow(DATA_COLUMNS_NAMES_C5C6 if len(csi_data) == len(DATA_COLUMNS_NAMES_C5C6) else DATA_COLUMNS_NAMES)
            header_written = True
