...
/* ---------------------------------------------------------------------------
 * Moved here from receiver/main/app_main.c so that the screen and the capture
 * are one program. receiver/ keeps the audited reference copy; this is the
 * living one. Everything that differs from it is listed here:
 *
 *   1. CONFIG_GAIN_CONTROL is no longer switched on for the ESP32-S3. The
 *      four esp_csi_gain_ctrl_* functions it calls do not exist in this SDK
 *      (they belong to Espressif's separate esp-csi component) and the build
 *      fails to link. With it off the readings are passed through unscaled -
 *      exactly what the classic ESP32 has always done, and those two columns
 *      are never printed in this CSV format anyway. The output is unchanged.
 *
 *   2. The queue holds 32 rows instead of 2. Two rows is 40ms at 50Hz, and
 *      anything slower than that - a screen repaint, a card write - lost
 *      readings. Now that the screen shares this board, that had to grow.
 *
 *   3. The output task is pinned to core 1, away from WiFi and the capture
 *      callback on core 0.
 *
 *   4. A row is formatted into one buffer and handed to csi_sink.h, instead
 *      of being printed a value at a time straight down the serial cable.
 *      That is what lets the same row go to the memory card instead.
 *
 *   5. The public functions are named csiCapture* to match the rest of this
 *      program (storageBegin, sensorBegin, drawHome). Same behaviour.
 *
 *   6. The "====== CSI RECV ======" banner is gone and the CSV header is now
 *      written once by csiCaptureStart(), not by the first row. A banner in
 *      front of the header would make a card file that no longer opens as a
 *      CSV - the first line has to be the column names and nothing else.
 * ------------------------------------------------------------------------ */
...
/* One row as text. Worst case is CSI_MAX_DATA_LENGTH values of up to seven
 * characters each plus the metadata columns. Real rows measured from this
 * project's own captures are 128 or 256 values, about 880 bytes. */
#define CSI_ROW_TEXT_MAX                  4096

#if CONFIG_IDF_TARGET_ESP32C5 || CONFIG_IDF_TARGET_ESP32C61
#define CSI_FORCE_LLTF                      0
#endif

/* ESP32-S3 deliberately absent - see note 1 at the top of this file. */
#if CONFIG_IDF_TARGET_ESP32C3 || CONFIG_IDF_TARGET_ESP32C5 || CONFIG_IDF_TARGET_ESP32C6 || CONFIG_IDF_TARGET_ESP32C61
...
const char* csiCaptureHeaderLine(void)
{
    ...
    return "type,id,mac,rssi,rate,noise_floor,fft_gain,agc_gain,channel,local_timestamp,sig_len,rx_format,len,first_word,data";
#else
    return "type,id,mac,rssi,rate,sig_mode,mcs,bandwidth,smoothing,not_sounding,aggregation,stbc,fec_coding,sgi,noise_floor,ampdu_cnt,channel,secondary_channel,local_timestamp,ant,sig_len,sig_mode,len,first_word,data";
#endif
}

/*
 * Formats one reading and hands it to whatever sink is installed.
 *
 * Only csi_output_task calls this, and there is only ever one of that task,
 * which is what makes the buffer below safe to keep between calls rather than
 * putting four kilobytes on a task's stack. It must never be called from the
 * capture callback.
 */
static void emit_csi_frame(const csi_frame_t *frame)
{
    static char row[CSI_ROW_TEXT_MAX];

    CsiRowSink sink = csiSinkGet();
    if (!sink) {
        return;
    }
    ...
    /*
     * Every append checks the one before it fitted. snprintf reports what it
     * WOULD have written, so a result that fills the space left means the row
     * was cut short - and half a reading passed on as though it were whole is
     * worse than no reading, so the row is dropped instead.
     */
    #define ROW_APPEND(...)                                                     \
        do {                                                                    \
            written = snprintf(row + used, sizeof(row) - (size_t)used,           \
                               __VA_ARGS__);                                     \
            if (written < 0 ||                                                   \
                (size_t)written >= sizeof(row) - (size_t)used) {                 \
                return;                                                          \
            }                                                                    \
            used += written;                                                     \
        } while (0)
...
#if CONFIG_IDF_TARGET_ESP32C5 || CONFIG_IDF_TARGET_ESP32C6 || CONFIG_IDF_TARGET_ESP32C61
    ROW_APPEND("CSI_DATA,%lu," MACSTR ",%d,%d,%d,%d,%d,%d,%d,%d,%d",
...
#else
    ROW_APPEND("CSI_DATA,%lu," MACSTR ",%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d",
               (unsigned long)frame->id, MAC2STR(frame->mac), frame->rx_ctrl.rssi,
    ...
    ROW_APPEND(",%d,%d,\"[%d", frame->len, frame->first_word_invalid, frame->data[0]);
    for (int index = 1; index < frame->len; index++) {
        ROW_APPEND(",%d", frame->data[index]);
    }
    ROW_APPEND("]\"");

    #undef ROW_APPEND

    sink(row, (size_t)used);
}
...
void csiCaptureStart(void)
{
    ...
    /* The column names go to the destination before any reading does, while
     * nothing else can be writing to it yet. */
    CsiRowSink sink = csiSinkGet();
    if (sink) {
        const char *header = csiCaptureHeaderLine();
        sink(header, strlen(header));
    }
...
uint32_t csiCaptureDrops(void)
{
    return capture_queue_drops;
}

uint32_t csiCaptureRows(void)
{
    return capture_frame_count;
}

bool csiCaptureIsConnected(void)
{
    return wifi_connected;
}

bool csiCaptureIsReady(void)
{
    return csi_ready;
}
...
static void wifi_csi_init()
{
    ...
    /* Core 1, away from WiFi and the capture callback on core 0. This is the
     * same core the screen draws on, so card writes and repaints take turns
     * rather than racing each other. */
    if (xTaskCreatePinnedToCore(csi_output_task, "csi_output", 4096, NULL, 2, NULL, 1) != pdPASS) {
        ESP_ERROR_CHECK(ESP_ERR_NO_MEM);
    }
